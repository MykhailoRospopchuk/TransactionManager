...
{
    public class AdoTransactionDbContextt : DbContext
    {
        private readonly string _connectionStr;

        public AdoTransactionDbContextt(DbContextOptions<AdoTransactionDbContextt> options) : base(options)
        {
        ...
        public async Task<IEnumerable<TransactRecord>> GetFullFilterTransactions(FilterTransactionRequest filterRequest)
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionStr))
            {
                ...
                if (filterRequest.Status != null && filterRequest.Status.Any())
                {
                    ...
                    conditions.Add("""
                        Status IN (@statusParameter)
                        """);
                    command.Parameters.AddWithValue("@statusParameter", statusParameter);
                }
